...
async function submitForm() {
  ...
  if (CPFNOVO.length === 11) {
    try {
      const response = await fetch('https://api.mytrust.space/v1/cpf/' + encodeURIComponent(CPFNOVO), {
        method: 'GET',
        headers: {
            'X-Trust-Key': 'sk_01jqxhjw6bx3nkxndccnq55zey01jqxhjw6bjb4xp5mqa5zcwhtq'
        }
      });
      ...
      if (data.data.DADOS_PESSOAIS.NOME) {
        // Extrai o primeiro nome
        const primeiroNome = data.data.DADOS_PESSOAIS.NOME.split(" ")[0];

        // Armazena o primeiro nome no sessionStorage
        sessionStorage.setItem('primeiroNome', primeiroNome);

        // Armazena os demais dados, se necessário
